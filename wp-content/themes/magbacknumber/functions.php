...
$mag_serial = ($_POST["mag_serial"] ?? "");
$mag_serial = substr(base64_decode($mag_serial), 2, -2);

// check is admin when login admin panel OR is user when have session
if (empty($mag_user) && empty($mag_serial)) {
	if (isLogin("admin") && timeout() >= 0) {
		$_SESSION["user"] = false;
		setTimeout();
		goto Start;
	}

	//when click view post in admin panel
	$URL_admin_panel = $_SERVER['HTTP_REFERER'] ?? "err";
	if (str_contains(get_site_url(), $URL_admin_panel)) {
		$_SESSION["admin"] = true;
		$_SESSION["user"] = false;
		setTimeout();
		goto Start;
	...
	if (isLogin("user") && timeout() >= 0) {
		// removeAllSession();
		// goto Err;
		$_SESSION["mag_category"] = setArgumentPost($_SESSION["mag_terms"], 1);
		$_SESSION["admin"] = false;
		setTimeout();
		goto Start;
	}
}
//check is user when click href
else {
	/*
	Table mag_terms
	1: 未分類
...
function setArgumentPost(string $category_name, string $post_number)
{
	// extract($arr);
	$arg = array(
...
function setArgumentSidebar(string $category_name, string $post_number, $page = null)
{
	// extract($arr);
	$arg = array(
		"category_name" => $category_name,
...
class MagazineWidget extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		if (@$_SESSION["user"] === true) {
			...
			// $new_args["category_name"] = $_SESSION["mag_category"]["category_name"];
			$_SESSION["mag_category"] = array(
				'category_name' => $_SESSION["mag_terms"]
				// 'cache_results' => false,
				// 'post_status' => 'publish',
			...
			global $post;
			$mag_terms = get_the_category($post->ID)[0]->name;
			// $new_args["category_name"] = $_SESSION["mag_category"];
			$_SESSION["mag_category"] = array(
				'category_name' => $mag_terms
				// 'cache_results' => false,
				// 'post_status' => 'publish',
				// 'nopaging' => false
			);
		...
		//     );
		// }
		//paging when login to admin panel
		...
		$latest_page = isset($_GET['latest_page']) ? intval($_GET['latest_page']) : 1;
		$new_args["paged"] = $latest_page;
		$new_args = setArgumentSidebar($_SESSION["mag_category"]["category_name"], $post_number, $latest_page);
...
function thongbao_init()
{
	...
	$new_args["paged"] = $latest_page;
	$new_args = setArgumentSidebar($magCate["category_name"], $post_number, $latest_page);
