<?php
    ...
    $mag_serial = str_replace("-", "", $mag_serial);
    $check_user = false;



    if(!function_exists("checkTF")){
        function checkTF($mag_serial){
            $json = '{
                "prod":{},
                "sral":{"query":"sral_no = \''.$mag_serial.'\'"}
            }';
    ...
    if ( !empty( $mag_user ) || !empty($mag_serial)) {
        ...
        */
        ...
        if($mag_category == "user"){
            if(!check_query($mag_child,$mag_user) && !checkTF($mag_serial))
                goto Err;
